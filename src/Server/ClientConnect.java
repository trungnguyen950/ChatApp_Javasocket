...
public class ClientConnect extends Thread {
	...
	public ServerApplication server;
	private String nickName;
	private DataOutputStream dos;
	private DataInputStream dis;
	private boolean run;
	
	public ClientConnect(ServerApplication server, Socket client){
		try {
			this.server=server;
			this.client=client;
		...
		while(run){
			...
			if(nickName.compareTo("0")==0){
				...
				if(checkNick(nickName)){
					...
					server.user.append(nickName+" connected\n");
					server.sendAll(nickName,nickName+" connected\n");
	...
	private void exit(){
		try {
			...
			dis.close();
			client.close();
			server.user.append(nickName+" disconnected\n");
			server.sendAll(nickName,nickName+" disconnected\n");
		} catch (IOException e) {
			e.printStackTrace();
		}	
	}
